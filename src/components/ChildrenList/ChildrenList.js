...
export default () => {
	const {project : {elements, activeComponent, element_menu}, activeElement} = useSelector(s => s);
	const dispatch = useDispatch();
	return (
		<div className={styles.children_list}>
			<div className={styles.path_container}>
				{activeElement && <Path element = {activeElement}/>}
			</div>
			<div className={styles.container}>
				<div>
					<ChildrenItem element ={activeComponent?.name} level = {0}/>
